...
abstract class OnjSchema(val nullable: Boolean) {
    abstract fun match(onjValue: OnjValue, parentName: String)
...
class OnjSchemaColor(nullable: Boolean) : OnjSchema(nullable) {

    override fun match(onjValue: OnjValue, parentName: String) {
        if (onjValue.isNull()) {
            if (nullable) return
            throw OnjSchemaException.fromNonNullable(parentName, "color")
        }
        if (!onjValue.isColor()) throw OnjSchemaException.fromTypeError(parentName, "color", getActualType(onjValue))
    }

    override fun getAsNullable(): OnjSchema {
        return OnjSchemaColor(true)
    }
}

class OnjSchemaVec2(nullable: Boolean) : OnjSchema(nullable) {

    override fun match(onjValue: OnjValue, parentName: String) {
        if (onjValue.isNull()) {
            if (nullable) return
            throw OnjSchemaException.fromNonNullable(parentName, "vec2")
        }
        if (!onjValue.isVec2()) throw OnjSchemaException.fromTypeError(parentName, "vec2", getActualType(onjValue))
    }

    override fun getAsNullable(): OnjSchema {
        return OnjSchemaVec2(true)
    }
}
